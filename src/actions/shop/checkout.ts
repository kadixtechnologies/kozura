...
/** Generates a unique order number prefixed with the first 2 alphanumeric chars of the store name.
 *  Example: store "Zuri Beauty" → "ZU-A3F8X2" */
function generateOrderNumber(storeName: string): string {
  const chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"; // unambiguous chars (no 0,O,I,1)
  // Extract first 2 alphanumeric characters from the store name, uppercase
  const prefix = (storeName.replace(/[^a-zA-Z0-9]/g, "").substring(0, 2) || "XX").toUpperCase();
  let suffix = "";
  for (let i = 0; i < 6; i++) {
...
export async function placeOrder(formData: FormData) {
  ...
  const { data: store } = await supabase.from("stores").select("name, whatsapp_number, slug, subscription_plan").eq("id", storeId).single();
  if (!store) {
  ...
  // Generate order number in application code (no DB trigger dependency)
  let orderNumber = generateOrderNumber(store.name);
  // Retry once on the very unlikely chance of a collision
  const { count: collision } = await supabase
    .from("orders")
    .select("id", { count: "exact", head: true })
    .eq("order_number", orderNumber);
  if (collision && collision > 0) {
    orderNumber = generateOrderNumber(store.name);
  }
